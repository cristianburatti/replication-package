...
All the required resources can be downloaded [here](https://zenodo.org/record/6603668#.Yph7Ri8RrJE).
...
are in the first folder of the resources you downloaded before. Put the `finetuning` folder you downloaded at 
`miner/data/finetuning` and run the following command:
...
`main.csv` and it can be found in the second folder. If you want to recreate exactly our dataset, use this file to
...
- tracing folders: these are 12 folders located in the third directory of the dataset. There exists one folder for each 
...
If you want to look at all the datasets we created for each of our models, you can find them under the fourth directory.
...
The resources required to run the notebooks can be found in the `pretrained model` folder you downloaded from Zenodo:
...
The fine-tuned models are also available in the resources you downloaded, under the `finetuned model` directory.
...
The folder created when we first run the script, and therefore the dataset we used in our thesis, is the penultimate 
directory. Note that we only provide the three CSV files and not all the repositories. If you want to download all of 
...
you downloaded at the beginning of this tutorial.
